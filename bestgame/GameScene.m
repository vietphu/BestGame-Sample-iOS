...
- (id) init
{
	if (self = [super init]) {
        ...
        if (payLoad && payLoad.activeFlag && payLoad.params) {
            NSDictionary *options = [payLoad.params objectForKey:PARAMS_OPTIONS_KEY];
            if (options && [options isKindOfClass:[NSDictionary class]] && [options objectForKey:OPTIONS_GAMETYPE_KEY]) {
                int option = [[options objectForKey:OPTIONS_GAMETYPE_KEY] integerValue];
                if (option == OPTIONS_GAMETYPE_SAMESY_VALUE) {
                    int seed = [[payLoad.params objectForKey:PARAMS_SEED_KEY] integerValue];
                    int round = [[payLoad.params objectForKey:PARAMS_ROUND_KEY] integerValue];
                    srandom(seed+round);
                    seeded = true;
                }
            }
